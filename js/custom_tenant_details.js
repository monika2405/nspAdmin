...
var table1 = $('#sledger').DataTable({
	...
	buttons: [{
		extend: 'excelHtml5',
		autoFilter: true,
		title: 'Laporan Ledger ' + window.location.href.split('?')[2] + '',
		sheetName: window.location.href.split('?')[2]
	}]
});

var table2 = $('#cledger').DataTable({
	"aLengthMenu": [
		[10, 20, -1],
		[10, 20, "All"]
	],
...
function countTotalContractBalance() {

	var totalContractDue = rem_fmoney($("#cDueTot").html());
	var totalContractReceived = rem_fmoney($("#cReceivedTot").html());
	var totalContractBalance = totalContractReceived - totalContractDue;
	$("#cBalanceTot").html(get_fmoney(totalContractBalance));

}

function countContractDue() {

	var totalDue = 0;
	for (i = 0; i < table2.rows().count(); i++) {
		var contractDue = rem_fmoney(table2.row(i).data()[2]);
		if (contractDue != null) {
			totalDue = totalDue + contractDue;
		}
	}
	$("#cDueTot").html(get_fmoney(totalDue));

}

function countContractReceived() {

	var totalReceived = 0;
	for (i = 0; i < table2.rows().count(); i++) {
		var contractReceived = rem_fmoney(table2.row(i).data()[3]);
		if (contractReceived != null) {
			totalReceived = totalReceived + contractReceived;
		}
	}
	$("#cReceivedTot").html(get_fmoney(totalReceived));

}

function countContractBalance() {
	var balance = 0;
	for (i = 0; i < table2.rows().count(); i++) {
		var contractDue = rem_fmoney(table2.row(i).data()[2]);
		if (contractDue != null) {
			balance = balance - contractDue;
		} else {
			var contractReceive = rem_fmoney(table2.row(i).data()[3]);
			balance = balance + contractReceive;
		}
		table2.cell(i, 4).data(get_fmoney(balance));
	}
}
...
function nonActive() {
	...
	setTimeout(function () {
		...
		firebase.database().ref().child("recurringPay/" + id).remove()
		firebase.database().ref().child("endingContract/" + id).remove()
		recurring.remove()
		//stop loading icon
...
function endContract() {
	...
	var floor_no = refNumb2.substring(3, 5)
	...
	var overPayment = rem_moneydot($("#overPaymentAmount").val())
	var totalCharge = rem_moneydot($("#totalChargeAmount").val())
	var property = firebase.database().ref("property/residential/" + "building_no:" + build_id + "/floor:" + floor_no + "/ID:" + room_id)
	var payment = firebase.database().ref("payment/" + id)
	var balance = rem_fmoney($("#cBalanceTot").text())
	...
	setTimeout(function () {
		...
		console.log(payplan2,balance)

		contract.child("historyperiod").once("value", function (snapshot) {
			var historyperiod = snapshot.val()
			var newhistoryperiod = ++historyperiod
			if (enddate2 == "Ongoing") {
				console.log(newhistoryperiod)
	...
	overdueRef.once('value', function (snapshot) {
		...
		if (prevDue == null) {
			...
			trRef1.once('child_added', function (snapshot) {
				...
				var rent = parseInt(snapshot.child("prorata_price").val());
				overdueRef.set({
...
function addPayment() {
	...
	var fpayRef = firebase.database().ref().child("first-payment/" + id);
	var virtual = firebase.database().ref("virtualAccountReport/" + id)
	var recurring = firebase.database().ref("recurringPay/" + id)
				...
				if (paymentDetails == "bondpay") { //bond money payment
					if (bondWaitDue - paymentAmount < 0) {
						if (bondWaitDue != 0) {
							...
							paymentRef.push({
								"date": paymentDate,
								"adjst_details": adjst_details,
								"adjst_date": adjst_date,
...
function extendTenant() {
	...
	var status = ""
	id = window.location.href.split('?')[1];
	var build_id = parseInt($("#extendBuildNo").text()).toString()
	var contract2 = firebase.database().ref().child("newContract/" + id + "");
	var recurringRef = firebase.database().ref("recurringPay/" + id + "/rental")

	contract2.on('child_added', function (snapshot) {
		room_id = snapshot.key
		contract2.child(room_id).on('value', function (snapshot) {
	...
	recurringRef.update({
		"payPlan": payPlan,
		"rent": rem_fmoney(rentPrice).toString()
	})
	...
	if (status == "inactive") {
		firebase.database().ref().child("HistoryRoom/" + build_id + "/" + tenantid4 + "/" + id).remove();
	}


	var newhistoryperiod = ++historyperiod
	var contract = firebase.database().ref().child("newContract/" + id + "/" + tenantid4 + "");
	contract.child(newhistoryperiod).update({
...
function doneEdit() {
	...
	if (isEmail($("#email_edt").val())) {
		...
		tenantRef.update({
			...
			setTimeout(function () {
				window.location = 'tenant_details.html?' + id + "?" + ref_num;
			}, 1000);
		...
		reftenantRef1.once('value', function (snapshot) {
			...
			reftenantRef2.once('value', function (snapshot) {
				...
				for (i = 0; i < (tenantNames.length); ++i) {
					...
					if ((tenantNames[i].tenantid) + "#ledger" == id + "#" + tab) {
...
$(document).ready(function () {
	...
	//start
	id = window.location.href.split('?')[1];
	ref_num = (window.location.href.split('?')[2]).split("#")[0];
	tab = (window.location.href.split('?')[2]).split("#")[1]

	bondp = 0
	rentp = 0
	var historyperiod = 0
	var status = ""
	var contract = firebase.database().ref().child("newContract/" + id + "");
	paymentRef = firebase.database().ref().child("payment/" + id);
...
`
	...
	trRef1.on('child_added', function (snapshot) {
		...
		var prorata = snapshot.child("prorata_price").val()
		var ctrt_opt = parseInt(snapshot.child("ctrt_opt").val());
		...
		if (payPlan == "annually") {
			...
			rentp = pembulatan((((prorata - 25000) / 1.1) * 2).toFixed(2));
			bondp = pembulatan((rentp / 12).toFixed(2));
			// console.log(rentp, bondp)
		} else if (payPlan == "monthly") {
			rentp = pembulatan((((prorata - 25000) / 1.2) * 12).toFixed(2));
			bondp = pembulatan((rentp / 12).toFixed(2));
		...
		if ((statOccupy == "approved") || (statOccupy == "active")) {
			...
			if (payPlan == "monthly"){
				contractList.push({
					"date": statingDate,
					"desc": "Rental Payment until End Contract",
					"invoice": null,
					"payment": (rentPrice * (ctrt_opt - 1)) + prorata
				})
				contractList.push({
					"date": statingDate,
					"desc": "Rental Due",
				...
				contractList = sortArrayByDate(contractList);
				console.log(contractList)
				for (x in contractList) {
					table2.row.add([reformatDate(contractList[x].date), contractList[x].desc, get_fmoney(contractList[x].invoice), get_fmoney(contractList[x].payment), null]);
				}
				table2.draw();
				//menghitung total contract
				countContractDue();
				countContractReceived();
				countContractBalance();
				countTotalContractBalance();
			}
	...
	var contractRef = firebase.database().ref("newContract/" + id);
	contractRef.on("child_added", function (snapshot) {
		var room_id = snapshot.key
		contractRef.child(room_id + "/2").once("value", function (snapshot) {
			var end_date = snapshot.child("end_date").val()
			var start_date = snapshot.child("start_date").val()
			var payPlan = snapshot.child("payPlan").val()
			var rent = snapshot.child("rent").val()
			var ctrt_length = parseInt(snapshot.child("ctrt_length").val())

			if (end_date != start_date && payPlan == "monthly" && end_date != "Ongoing") {
				contractList.push({
					"date": start_date,
					"desc": "Rental Payment until End Contract",
					"invoice": null,
					"payment": rent * ctrt_length
				})
				table2.clear();
	...
	$("#startDate").on('change', function () {
		$("#ExtendstartDate").html($("#startDate").val())
	})
	...
	//end contract form penalty price amount listener
	$("#overPaymentAmount").on('keyup change', function () {
		$("#overPaymentAmount").val(get_moneydot($("#overPaymentAmount").val()));
	})
	//end contract form penalty price amount listener
	$("#totalChargeAmount").on('keyup change', function () {
		$("#totalChargeAmount").val(get_moneydot($("#totalChargeAmount").val()));
	})
	...
	contractLedger = firebase.database().ref("contractLedger/" + id)
	console.log(id)
	contractLedger.on("child_added", function (snapshot) {
		var key_id = snapshot.key
		contractLedger.child(key_id).on("value", function (snapshot) {
			var date = snapshot.child("date").val()
