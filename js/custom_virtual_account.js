...
$(document).ready(function () {
        ...
        trRef.child(tenant_id).on('child_added', function (snapshot) {
            ...
            trRef.child(tenant_id + "/" + room_id).on('value', function (snapshot) {
                ...
                tenantRef.child(tenant_id).on('value', function (snapshot) {
                    ...
                    virtual.child(tenant_id + "/balance").on('value', function (snapshot) {
                        if (snapshot.val()<0){
                            virtualList[tenant_id] = {
                                "due": snapshot.val().split("-")[1],
    ...
    setTimeout(() => {
        ...
        for (i in virtualList) {
            ...
            table7.row.add([virtualList[i].build_no, virtualList[i].ref_number, "<a href='tenant_details.html?"+i+"?"+virtualList[i].ref_number+"' class='pull-left'>"+virtualList[i].full_name+"</a>" , get_fmoney(virtualList[i].due)]).node().id = i;
            //}
    ...
    //             // year = i.split("-")[1].split(":")[1]
    //             // if (month == thisMonth && year == thisYear) {
    //             table7.row.add([virtualList[i].build_no, virtualList[i].ref_number, virtualList[i].full_name, get_fmoney(virtualList[i].due)]).node().id = i;
    //             //}
