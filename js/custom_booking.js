					...
					if (snapshot.val() != null) {  // Have payments
						...
						if(statOccupy=="booking") {
							...
							newObj = {
								"statOccupy":"booking",
								"refNum":refNumber,
								"content":[refNumFormat,statingDate,propAddr,buildNo,floorNo,roomNo,"<a href='tenant_approve.html?id="+refNumber+"' class='pull-left'>"+tenantName+"</a>",tenantContact,"<i class='fa fa-check' style='color:#c8bca6'></i></button> <button id='removebutt' class='btn btn-xs btn-danger' title='Delete' onclick=deleteBooking('"+tenantID+"')><i class='fa fa-times'></i></button>"],
								"tenant_id":tenantID
							}
							listApproveT.push(newObj);
						}
